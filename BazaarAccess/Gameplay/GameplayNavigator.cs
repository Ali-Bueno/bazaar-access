...
public class GameplayNavigator
{
    ...
    /// <summary>
    /// Reads out the current challenges (daily and weekly).
    /// Refreshes data from server first to get up-to-date progress.
    /// </summary>
    public void ReadChallenges()
    {
        TolkWrapper.Speak("Loading challenges...");
        Plugin.Instance.StartCoroutine(ReadChallengesCoroutine());
    }

    private System.Collections.IEnumerator ReadChallengesCoroutine()
    {
        // Refresh challenge data from server
        var refreshTask = GameInstance.Instance?.ProfileDataProvider?.RefreshPlayerChallengesData(forceRefresh: true);
        if (refreshTask != null)
        {
        ...
        try
        {
            var challengesProfile = Data.ChallengesProfile;
            if (challengesProfile == null)
            {
                TolkWrapper.Speak("Challenges not available");
                yield break;
            }
            ...
            {
                TolkWrapper.Speak("Challenge data not available");
                yield break;
            }
            ...
            // Daily challenges
            var dailyChallenges = challengesProfile.DailyChallenges;
            if (dailyChallenges != null && dailyChallenges.Length > 0)
            {
                parts.Add($"{dailyChallenges.Length} daily challenges");
                foreach (var challenge in dailyChallenges)
                {
                    string info = GetChallengeInfo(challengeManager, challenge);
            ...
            // Weekly challenges
            var weeklyChallenges = challengesProfile.WeeklyChallenges;
            if (weeklyChallenges != null && weeklyChallenges.Length > 0)
            {
                parts.Add($"{weeklyChallenges.Length} weekly challenges");
            ...
            if (parts.Count == 0)
            {
                TolkWrapper.Speak("No active challenges");
            }
            else
            {
                TolkWrapper.Speak(string.Join(". ", parts));
        ...
        {
            Plugin.Logger.LogError($"ReadChallenges error: {ex.Message}");
            TolkWrapper.Speak("Cannot read challenges");
        }
    }

    private string GetChallengeInfo(ChallengeDataManager manager, BazaarGameShared.TempoNet.Responses.ChallengeProgress challenge)
    {
        try
        {
            BazaarGameShared.Domain.Challenges.TChallenge challengeData;
            if (!manager.TryGetChallenge(challenge.Id.ToString(), out challengeData))
            {
                return null;
            }

            // Get localized title and description using LocalizationService
            var locService = Services.Get<TheBazaar.Localization.LocalizationService>();
            string title = "Unknown";
            string description = "";

            if (challengeData.Localization?.Title != null)
            {
                if (locService != null && locService.TryGetText(challengeData.Localization.Title, out var localizedTitle))
                {
                    title = localizedTitle;
                }
                else
                {
                    title = challengeData.Localization.Title.Text ?? "Unknown";
                }
            }

            if (challengeData.Localization?.Description != null)
            {
                if (locService != null && locService.TryGetText(challengeData.Localization.Description, out var localizedDesc))
                {
                    description = localizedDesc;
                }
                else
                {
                    description = challengeData.Localization.Description.Text ?? "";
                }
            }

            // Replace {completionRequirement} with actual value
            if (description.Contains("{completionRequirement}"))
            {
                description = description.Replace("{completionRequirement}", challengeData.CompletionRequirement.ToString());
            }

            int progress = challenge.Progress;
            int requirement = challengeData.CompletionRequirement;
            bool complete = progress >= requirement;
            bool claimed = challenge.Acknowledged;
            ...
            {
                status = "complete, ready to claim";
            }
            else
            {
                status = $"{progress} of {requirement}";
            }
